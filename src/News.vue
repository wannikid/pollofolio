  ...
  <v-expansion-panel ripple>
    ...
    <v-expansion-panel-content>
      ...
      <v-list dense two-line v-if="uniqueNews.length > 0" class="mt-3">
        <template v-for="(item, idx) of uniqueNews">
          <v-list-item :key="idx" class="px-0" :href="item.url" target="_blank">
            <v-list-item-content>
              <v-list-item-subtitle class="text-truncate overline">{{ convertDate(item.datetime) }}</v-list-item-subtitle>
          ...
          <v-divider v-if="idx + 1 < uniqueNews.length" :key="'div-' + idx"></v-divider>
        </template>
      </v-list>
    </v-expansion-panel-content>
  </v-expansion-panel>
